...
description: >
  ...
  identical and sacred — the five-tone warm-stone palette, Audiowide 400 + Audiowide, the
...
typography:
  # — reading ramp (Audiowide) —
  body:        { fontFamily: "Space Mono", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "Space Mono", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "Space Mono", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "Space Mono", px: 14, weight: 500, tracking: "3px", upper: true, color: "text-secondary" }
  attribution: { fontFamily: "Space Mono", px: 15, weight: 400, tracking: "2px", upper: true, color: "text-secondary" }
  micro:       { fontFamily: "Space Mono", px: 12, weight: 400, tracking: "2px", upper: true, color: "text-secondary" }
  # — display / hero ramp (Audiowide 400, sentence case) —
  h3:          { fontFamily: "Audiowide", cqw: 1.8, weight: 400, lineHeight: 1.1 }
  timeline-headline:{ fontFamily: "Audiowide", cqw: 1.9, weight: 400, lineHeight: 1.1 }
  card-headline:{ fontFamily: "Audiowide", cqw: 2.0, weight: 400, lineHeight: 1.15 }
  stat-figure: { fontFamily: "Audiowide", cqw: 3.0, weight: 400, lineHeight: 1.0 }
  quote-mark:  { fontFamily: "Audiowide", cqw: 9.0, weight: 400, lineHeight: 0.5, color: "accent" }
  h2:          { fontFamily: "Audiowide", cqw: 4.0, weight: 400, lineHeight: 1.1 }
  h1:          { fontFamily: "Audiowide", cqw: 6.2, weight: 400, lineHeight: 1.06 }
  display:     { fontFamily: "Audiowide", cqw: 8.0, weight: 400, lineHeight: 1.04 }
...
components:
  ...
  stats-cluster:
    borderTop: "0.07cqw solid {colors.line}"
    typography: "{typography.stat-figure} + uppercase {colors.text-secondary} labels"
...
- **Fonts** — already set to **Audiowide** (display) / **Space Mono** (body); ignore any preset font name lingering in prose.
...
The voice is a literary pairing: **Audiowide** at weight 400 (the thin-stroke didone, never
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **Audiowide**
...
- **Audiowide 400** (ink, sentence case) for display; **Audiowide** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (Audiowide body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **Audiowide labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs Audiowide sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an Audiowide subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **Audiowide + Audiowide via Google Fonts.** CJK pairing (Noto Serif SC 700/400) carries over; Playfair has no Hanzi italic — substitute weight/taupe for emphasis.
...
```html
<style>
@font-face{font-family:"Audiowide";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/Audiowide-Regular.ttf") format("truetype");}
@font-face{font-family:"Space Mono";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/SpaceMono-Regular.ttf") format("truetype");}
</style>
```
